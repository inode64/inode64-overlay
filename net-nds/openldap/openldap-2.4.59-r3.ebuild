...
src_configure() {
	...
	# 2.4.x predates C23. GCC >= 15 defaults to -std=gnu23, where an empty
	# parameter list means (void), so the K&R-style fallback declarations in
	# include/ac/string.h ("int (strcasecmp)();", "char *(strdup)();") clash
	# with the real glibc prototypes:
