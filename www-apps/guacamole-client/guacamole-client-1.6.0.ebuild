...
# tar -xf guacamole-client-1.6.0.tar.gz (don't use mc!!)
...
# Compression process:
# cd ..
# tar --create --auto-compress --file guacamole-client-1.6.0-mvn.tar.xz guacamole-client-1.6.0/.m2
# tar --create --auto-compress --file guacamole-client-1.6.0-node_modules.tar.xz guacamole-client-1.6.0/guacamole/src/main/frontend/node_modules guacamole-client-1.6.0/guacamole-common-js/node

if [[ "${PV}" == *9999 ]] ; then
	inherit git-r3
	EGIT_REPO_URI="https://github.com/apache/guacamole-client.git"
	EGIT_BRANCH="staging/1.6.0"
else
...
SRC_URI+="
	...
	mysql? ( https://dev.mysql.com/get/Downloads/Connector-J/mysql-connector-j-8.3.0.tar.gz )
...
src_unpack() {
	...
	if use mysql; then
		unpack mysql-connector-j-8.3.0.tar.gz
	fi
}
...
src_install() {
	...
	if use mysql; then
		...
		doins "${WORKDIR}"/mysql-connector-j-8.3.0/mysql-connector-j-8.3.0.jar
