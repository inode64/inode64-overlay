...
_rocm_set_globals() {
	...
	case ${ROCM_VERSION} in
		...
		10.*)
			...
			official_amdgpu_targets=(
				gfx908 gfx90a gfx942 gfx950 gfx1030
...
rocm_use_clang() {
	...
	if [[ ${PV} == 10.* ]]; then
		# Clang's HIP detector assumes a single "lib" directory.  Present a
		# private SDK view so Gentoo's native lib64 layout remains intact.
		local hip_sdk_root="${T}/gentoo-hip-sdk"
		mkdir -p "${hip_sdk_root}/share/hip" || die
		ln -sfn "${EPREFIX}/usr/include" "${hip_sdk_root}/include" || die
		ln -sfn "${EPREFIX}/usr/$(get_libdir)" "${hip_sdk_root}/lib" || die
		ln -sfn "${EPREFIX}/usr/share/hip/version" \
			"${hip_sdk_root}/share/hip/version" || die
		append-cxxflags "--hip-path=${hip_sdk_root}"
	fi
}
